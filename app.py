...
def main():
    ...
    with tab2:
        ...
        if not df.empty:
            # Convertiamo la colonna Data per poterla filtrare
            # Il formato nel foglio è "YYYY-MM-DD HH:MM:SS"
            ...
            if not df_filt.empty:
                ...
                st.markdown("---")
                with st.expander("📋 Vedi Dettagli Completi", expanded=True):
                    # Mostriamo i dati in modo più carino, escludendo colonne tecniche
                    display_data = row.drop(['Data_dt'], errors='ignore').to_dict()
